...
const allowMenuItems = ['admin', 'user-registration', 'test', 'test.comp1', 'test.comp2', 'test.comp2.comp4'];

export const ROUTES: RouteInfo[] = [

  {key: 'infoadd', path: '/infoadd', title: 'Dashboard', icon: 'dashboard', class: ''},
  {key: 'add', path: '/add', title: 'Dashboard', icon: 'dashboard', class: ''},
  {key: 'admin', path: '/admin', title: 'Admin', icon: 'assignment_ind', class: '', children:
      [
        {key: 'user-registration', path: '/admin/user-registration', title: 'User registration', icon: 'how_to_reg', class: ''}
      ]
  },
  {
  ...
  ngOnInit() {
    this.menuItems = ROUTES.filter(menuItem => {
      if (allowMenuItems.includes(menuItem.key)) {// check whether main menu authorized, include in the list
        if (menuItem.children) {// if there are children then check there permission separately
  ...
  removeUnauthorizedSubMenuItems(menuItem: RouteInfo): RouteInfo {
    ...
    menuItem.children = menuItem.children.filter(submenuItem => {
      if (allowMenuItems.includes(submenuItem.key)) {
        ...
        if (submenuItem.children) {// if there are children for first level submenu and so on, then check there permission separately
          return this.removeUnauthorizedSubMenuItems(submenuItem);
        } else {
          return submenuItem;
        }
      } else { // no else path as this filter copy one list to another. as sub menuitem not in the list no need to copy
      }
    });
